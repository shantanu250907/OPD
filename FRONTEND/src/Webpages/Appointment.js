...
//     // Check if appointment is locked (Cancelled or Completed)
//     if (apt.status === 'Cancelled' || apt.status === 'Completed') {
//       alert(`❌ Cannot edit: This appointment is ${apt.status} and locked!`);
//       return;
//     }
...
//     // ✅ FIXED: Allow deletion of Completed and Cancelled appointments
//     // No restriction - all appointments can be deleted
...
//   // ==================== handleStatusChange with enhanced restrictions ====================
...
//       // ✅ ENHANCED STATUS RESTRICTIONS - COMPLETE LOCKING
//       // 1. If status is CANCELLED, completely lock it - cannot change to anything
//       if (oldStatus === 'Cancelled') {
//         alert("❌ Cannot change status: This appointment is CANCELLED and locked!");
//         return;
//       }
      
//       // 2. If status is COMPLETED, cannot change to anything (including Cancelled)
//       if (oldStatus === 'Completed') {
//         alert("❌ Cannot change status: This appointment is COMPLETED and locked!");
//         return;
//       }
      
//       // 3. If trying to change from Pending to Cancelled or Completed - allowed
//       if (oldStatus === 'Pending' && (newStatus === 'Completed' || newStatus === 'Cancelled')) {
//         // Allowed transitions
//       }
//       // 4. Block any other transitions
//       else if (!(oldStatus === 'Pending' && (newStatus === 'Completed' || newStatus === 'Cancelled'))) {
...
//           alert("✅ Appointment cancelled and locked! This appointment cannot be modified further.");
//         } else if (newStatus === 'Completed') {
//           alert("✅ Appointment marked as completed and locked!");
//         }
//       } else {
...
//                           opacity: apt.status === 'Cancelled' || apt.status === 'Completed' ? 0.7 : 1
//                         }}
...
//                         <small style={{ display: 'block', color: apt.status === 'Cancelled' ? '#dc3545' : '#28a745', fontSize: '10px', marginTop: '3px' }}>
//                           🔒 Locked
//                         </small>
//                       )} */}
//                     </div>
...
//                       title={apt.status === 'Cancelled' || apt.status === 'Completed' ? "Cannot edit locked appointment" : "Edit Appointment"}
//                       style={{
//                         background: "none",
//                         border: "none",
//                         cursor: apt.status === 'Cancelled' || apt.status === 'Completed' ? "not-allowed" : "pointer",
  ...
  const handleStatusChange = async (apt, newStatus) => {
    try {
      ...
      if (response.ok && data.success) {
        ...
        if (newStatus === 'Cancelled') {
          alert("✅ Appointment cancelled and locked!");
        } else if (newStatus === 'Completed') {
          alert("✅ Appointment marked as completed and locked!");
        }
      } else {
  ...
  return (
    <div className="appointments-page">
      ...
      <div className="summary-stats">
        <div 
          className={`summary-card ${filterType === 'all' ? 'active-filter' : ''}`}
          style={{ borderLeft: "4px solid #0d6efd", cursor: "pointer", transition: "all 0.2s", opacity: filterType === 'all' ? 1 : 0.8, transform: filterType === 'all' ? 'scale(1.02)' : 'scale(1)' }}
          onClick={() => handleFilterClick('all')}
          title={`Click to show all appointments (${getFilteredCount('all')} appointments)`}
        >
          <h4>📅 TOTAL</h4>
        ...
        <div 
          className={`summary-card ${filterType === 'pending' ? 'active-filter' : ''}`}
          style={{ borderLeft: "4px solid #ffc107", cursor: "pointer", transition: "all 0.2s", opacity: filterType === 'pending' ? 1 : 0.8, transform: filterType === 'pending' ? 'scale(1.02)' : 'scale(1)' }}
          onClick={() => handleFilterClick('pending')}
          title={`Click to show pending appointments (${getFilteredCount('pending')} appointments)`}
        >
          <h4>⏳ PENDING</h4>
        ...
        <div 
          className={`summary-card ${filterType === 'completed' ? 'active-filter' : ''}`}
          style={{ borderLeft: "4px solid #28a745", cursor: "pointer", transition: "all 0.2s", opacity: filterType === 'completed' ? 1 : 0.8, transform: filterType === 'completed' ? 'scale(1.02)' : 'scale(1)' }}
          onClick={() => handleFilterClick('completed')}
          title={`Click to show completed appointments (${getFilteredCount('completed')} appointments)`}
        >
          <h4>✔ COMPLETED</h4>
        ...
        <div 
          className={`summary-card ${filterType === 'cancelled' ? 'active-filter' : ''}`}
          style={{ borderLeft: "4px solid #dc3545", cursor: "pointer", transition: "all 0.2s", opacity: filterType === 'cancelled' ? 1 : 0.8, transform: filterType === 'cancelled' ? 'scale(1.02)' : 'scale(1)' }}
          onClick={() => handleFilterClick('cancelled')}
          title={`Click to show cancelled appointments (${getFilteredCount('cancelled')} appointments)`}
        >
          <h4>❌ CANCELLED</h4>
          ...
          <tbody>
            {filteredAppointments.length > 0 ? (
              filteredAppointments.map((apt, index) => (
                <tr key={apt._id || apt.id || index}>
                  <td>{index + 1}</td>
                  <td><strong>{apt.patientName}</strong><br/><small style={{color:"#666"}}>{apt.phone}</small></td>
                  ...
                  <td>
                    <select
                      ...
                      style={{
                        ...
                        fontWeight: "600",
                        cursor: apt.status === 'Cancelled' || apt.status === 'Completed' ? "not-allowed" : "pointer",
                        backgroundColor: apt.status === "Pending" ? "#fff3cd" : apt.status === "Completed" ? "#d4edda" : "#f8d7da",
                        color: apt.status === "Pending" ? "#856404" : apt.status === "Completed" ? "#155724" : "#721c24",
                        borderColor: apt.status === "Pending" ? "#ffeeba" : apt.status === "Completed" ? "#c3e6cb" : "#f5c6cb",
                        opacity: apt.status === 'Cancelled' || apt.status === 'Completed' ? 0.7 : 1
                      }}
                      disabled={apt.status === 'Cancelled' || apt.status === 'Completed'}
                    >
                  ...
                  <td className="action-cell">
                    <button className="view-btn" onClick={() => handleView(apt)} title="View Details" style={{background: "none", border: "none", cursor: "pointer", fontSize: "18px", padding: "5px"}}>👁️</button>
                    <button className="edit-btn" onClick={() => handleEdit(apt)} title={apt.status === 'Cancelled' || apt.status === 'Completed' ? "Cannot edit locked appointment" : "Edit Appointment"} style={{background: "none", border: "none", cursor: apt.status === 'Cancelled' || apt.status === 'Completed' ? "not-allowed" : "pointer", fontSize: "18px", padding: "5px", opacity: apt.status === 'Cancelled' || apt.status === 'Completed' ? 0.5 : 1}} disabled={apt.status === 'Cancelled' || apt.status === 'Completed'}>✏️</button>
                    <button className="delete-btn" onClick={() => handleDelete(apt)} title="Delete Appointment" style={{background: "none", border: "none", cursor: "pointer", fontSize: "18px", padding: "5px"}}>❌</button>
                  </td>
                </tr>
              ))
            ) : (
              <tr>
                <td colSpan="8" style={{ textAlign: "center", padding: "30px", color: "#666" }}>
                  {searchTerm ? `No appointments found matching "${searchTerm}"` : filterType !== 'all' ? `No ${filterType} appointments found` : "No appointments found"}
                </td>
              </tr>
            )}
          </tbody>
      ...
      {/* VIEW POPUP - Same as DoctorAppointments */}
      {showViewPopup && selectedAppointment && (
      ...
      {/* EDIT POPUP */}
      {showEditPopup && selectedAppointment && (
        <div className="popup-overlay" onClick={() => setShowEditPopup(false)} style={{position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", display: "flex", justifyContent: "center", alignItems: "center", zIndex: 1000}}>
          <div className="popup-content" onClick={(e) => e.stopPropagation()} style={{width: "600px", maxHeight: "85vh", overflowY: "auto", background: "#fff", padding: "30px", borderRadius: "12px", boxShadow: "0 20px 60px rgba(0,0,0,0.3)"}}>
            <div style={{display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px"}}>
              <h2 style={{margin: 0, color: "#2c3e50"}}>✏️ Edit Appointment</h2>
              <button onClick={() => setShowEditPopup(false)} style={{background: "none", border: "none", fontSize: "24px", cursor: "pointer", color: "#666"}}>×</button>
            </div>
            <form onSubmit={handleSave}>
              <div style={{display: "flex", flexDirection: "column", gap: "15px"}}>
                <div style={{background: "#f8f9fa", padding: "15px", borderRadius: "8px"}}>
                  ...
                  <div style={{display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px"}}>
                    <div style={{gridColumn: "span 2"}}>
                      <input name="patientName" placeholder="Patient Name *" value={formData.patientName || ""} onChange={handleEditChange} style={{...inputStyle, ...(editErrors.patientName ? errorStyle : {})}} required />
                      {editErrors.patientName && <span style={{color: "#dc3545", fontSize: "12px", marginTop: "3px", display: "block"}}>{editErrors.patientName}</span>}
                    </div>
                    <div><input name="age" type="number" placeholder="Age *" value={formData.age || ""} onChange={handleEditChange} min="1" max="120" style={{...inputStyle, ...(editErrors.age ? errorStyle : {})}} required /></div>
                    <div>
                      <select name="gender" value={formData.gender || ""} onChange={handleEditChange} style={{...inputStyle, ...(editErrors.gender ? errorStyle : {})}} required>
                    ...
                    <div style={{gridColumn: "span 2"}}><input name="phone" placeholder="Phone *" value={formData.phone || ""} onChange={handleEditChange} maxLength="10" style={{...inputStyle, ...(editErrors.phone ? errorStyle : {})}} required /></div>
                    <div style={{gridColumn: "span 2"}}><input name="email" type="email" placeholder="Email *" value={formData.email || ""} onChange={handleEditChange} style={{...inputStyle, ...(editErrors.email ? errorStyle : {})}} required /></div>
                  </div>
                </div>
                <div style={{background: "#f8f9fa", padding: "15px", borderRadius: "8px"}}>
                  <h3 style={{margin: "0 0 15px 0", color: "#0d6efd", fontSize: "16px"}}>Symptoms (Optional)</h3>
                  <div className="symptoms-container">
                    <div className="symptoms-select-box" onClick={() => setSymptomsDropdownOpen(!symptomsDropdownOpen)} style={{padding: "10px", border: "1px solid #ddd", borderRadius: "6px", cursor: "pointer", backgroundColor: "#fff", display: "flex", justifyContent: "space-between", alignItems: "center"}}>
                      <div className="selected-symptoms-preview">
                        {formData.symptoms?.length > 0 ? (
                          <div style={{display: "flex", flexWrap: "wrap", gap: "5px"}}>
                            {formData.symptoms.slice(0, 2).map((symptom) => (
                              <span key={symptom} style={{background: "#e3f2fd", padding: "4px 8px", borderRadius: "16px", fontSize: "12px", display: "inline-flex", alignItems: "center", gap: "4px"}}>
                                {symptom}
                                <button type="button" onClick={(e) => { e.stopPropagation(); handleSymptomChange(symptom); }} style={{background: "none", border: "none", cursor: "pointer", fontSize: "14px", padding: "0 2px"}}>×</button>
                              </span>
                            ))}
                            {formData.symptoms.length > 2 && <span style={{color: "#666", fontSize: "12px"}}>+{formData.symptoms.length - 2} more</span>}
                          </div>
                        ) : <span style={{color: "#999"}}>Select symptoms</span>}
                      </div>
                      <span style={{fontSize: "12px", color: "#666"}}>▼</span>
                    ...
                    {symptomsDropdownOpen && (
                      <div style={{marginTop: "10px", padding: "10px", border: "1px solid #ddd", borderRadius: "6px", maxHeight: "200px", overflowY: "auto", backgroundColor: "#fff"}}>
                        {cardiologySymptoms.map((symptom) => (
                          <label key={symptom} style={{display: "block", padding: "5px", cursor: "pointer"}}>
                            <input type="checkbox" checked={formData.symptoms?.includes(symptom) || false} onChange={() => handleSymptomChange(symptom)} style={{marginRight: "8px"}} />
                            {symptom}
                          </label>
                        ))}
                      </div>
                ...
                <div style={{background: "#f8f9fa", padding: "15px", borderRadius: "8px"}}>
                  <h3 style={{margin: "0 0 15px 0", color: "#0d6efd", fontSize: "16px"}}>Appointment Details</h3>
                  <div style={{display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px"}}>
                    <div><input name="date" type="date" placeholder="Date *" value={formData.date || ""} onChange={handleEditChange} min={new Date().toISOString().split('T')[0]} style={{...inputStyle, ...(editErrors.date ? errorStyle : {})}} required /></div>
                    <div><input name="time" type="time" placeholder="Time *" value={formData.time || ""} onChange={handleEditChange} style={{...inputStyle, ...(editErrors.time ? errorStyle : {})}} required /></div>
                    <div style={{gridColumn: "span 2"}}><textarea name="notes" placeholder="Additional notes..." value={formData.notes || ""} onChange={handleEditChange} rows="3" style={{...inputStyle, resize: "vertical"}} /></div>
                  </div>
                </div>
              </div>
              <div style={{marginTop: "25px", display: "flex", justifyContent: "flex-end", gap: "10px"}}>
                <button type="button" onClick={() => setShowEditPopup(false)} style={{background: "linear-gradient(135deg, #6c757d, #5c636a)", color: "#fff", padding: "10px 25px", border: "none", borderRadius: "8px", cursor: "pointer", fontWeight: "600"}}>Cancel</button>
                <button type="submit" style={{background: "linear-gradient(135deg, #28a745, #218838)", color: "#fff", padding: "10px 25px", border: "none", borderRadius: "8px", cursor: "pointer", fontWeight: "600"}} disabled={isLoading}>{isLoading ? "Saving..." : "Save Changes"}</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {showPopup && (
        <AppointmentForm
          onClose={() => { setShowPopup(false); setSelectedPatientForAppointment(null); }}
          addAppointment={addAppointment}
          appointments={localAppointments}
          initialPatientData={selectedPatientForAppointment}
        />
